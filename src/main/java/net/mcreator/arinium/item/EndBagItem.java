...
import net.mcreator.arinium.gui.EndBagGuiGuiWindow;
import net.mcreator.arinium.gui.EndBagGuiGui;
...
public class EndBagItem extends AriniumModElements.ModElement {
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof EndBagGuiGuiWindow) {
	...
	public static class ItemCustom extends Item {
		public ItemCustom() {
			super(new Item.Properties().group(UtilitiesItemGroup.tab).maxStackSize(1).rarity(Rarity.COMMON));
			setRegistryName("end_bag");
		}
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("End Bag");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new EndBagGuiGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
